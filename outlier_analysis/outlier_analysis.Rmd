...
stops<-stops%>%
  ...
  ## all analysis variables appear with column header .x because they have been duplicated for some reason. If you wish to figure out how to unduplicate, remember to remove the ".x" from the end of each analysis variable in the code below.

# select columns of interest

final_cap<-stops%>%
select(c("stop_id", "stop_duration", "pv.x", "lwr.x", "upr.x", "pv_log.x", "upr_log.x", "lwr_log.x"))
...
final_cap<-final_cap%>%
 mutate(cap_type=ifelse(stop_duration>upr.x, "high",
                         ifelse(stop_duration<lwr.x, "low", "none")))%>%
  mutate(stop_duration_capped=ifelse(cap_type=="high", upr.x,
                              ifelse(cap_type=="low", lwr.x,
                                     stop_duration)))

# rearrange column order 

col_order <- c("stop_id", "stop_duration", "stop_duration_capped", "cap_type",  "pv.x", "lwr.x","upr.x", "pv_log.x", "lwr_log.x", "upr_log.x")
