...
# First set the datatable to be at the stop level where each gender for each stop (where multiple people were stopped in a singel stop) show up but as a single row

# Then recode data so that for stops that have multiple people and multiple genders, a ne wflag 'g_multiple' == 1 to indicate that the stop has multiple genders
...
g_female+
    g_transgender_man+
     g_transgender_woman+
     # g_multigender+ # Removing this from the model because this variable is aliased when running vif score probably due to addition of g_multiple column from gender recoding
...
stop_reason_simple_parole+
 #  stop_reason_simple_reasonable_suspicion+ # Updated model makes this variable have a vif = 5.155970 which is high. There are only 239 stops with stop_reason_simple_reasonable_suspicion==1 
...
# stop_result_simple_citation--make reference because it's a larger category and added stop_result_simple_no_action
