...
# 9/12/24: JZ alternate code for action_more that runs faster ---PLEASE QA

action_more<-action_person %>%
   group_by(stop_id) %>%
    mutate(across(removed_from_vehicle:ads_written_statement, max)) %>%  # I use a sum(is.na(person_action)) to make sure there are no NAs in the df and then I remove the na.rm=TRUE arguement for speed. I also specify the columns I am mutating across
  slice(1) %>%
  select(-person_number, -actions_count, -action_taken,
         -removed_from_vehicle, -handcuffed, -detained, -use_of_force)
...
# 9/12/24: JZ alternative code that runs faster---PLEASE QA
...
vars_remove<-grep("stop_nh_race_nh_white|stop_reason_simple_traffic_violation|stop_result_simple_no_action|g_male",  colnames(stops_d))
...
vif_values # removed actions_count, removed_vehicle_order, contraband_found, stop_reason_simple_citerelease, all VIF values now below 5.
...
The current model has a R^2 of 54% which is a good indication that our model is robust. This means that 52% of variance in stop duration times is explained by our model. 
...
## JZ commenting out code. Don't need the renaming. I don't see the .x in the column names when I run Isaac's
## original code. 
