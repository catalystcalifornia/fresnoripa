###Analysis: Traffic stops by citation stop result
...
df1_aian<-df%>%
  filter(reason=="Traffic violation" & stop_result_simple=="citation for infraction" & aian_flag==1)%>%
  mutate(total=n())%>%
  group_by(statute_literal_25, offense_type_of_charge)%>%
  mutate(count=n(),
         rate=count/total*100)%>%
...
df1_nhpi<-df%>%
  filter(reason=="Traffic violation" & stop_result_simple=="citation for infraction" & nhpi_flag==1)%>%
  mutate(total=n())%>%
  group_by(statute_literal_25, offense_type_of_charge)%>%
  mutate(count=n(),
         rate=count/total*100)%>%
...
df1_sswana<-df%>%
  filter(reason=="Traffic violation" & stop_result_simple=="citation for infraction" & sswana_flag==1)%>%
  mutate(total=n())%>%
  group_by(statute_literal_25, offense_type_of_charge)%>%
  mutate(count=n(),
         rate=count/total*100)%>%
