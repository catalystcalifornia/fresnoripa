...
# Analysis: Top Citations NOT by stop type-------------------------------------------
...
df_nhpi<-df%>%
  ...
  group_by(traffic_violation_type)%>%
  mutate(total=n())%>%
  left_join(offense_codes, by=c("offense_code"="offense_code"))%>%  group_by(traffic_violation_type, statute_literal_25, offense_type_of_charge)%>%
  mutate(count=n(),
         rate=count/total*100)%>%
  slice(1)%>%
ungroup()%>%
