...
*All people in Fresno deserve to feel safe. However, Fresno PD uses traffic stops to profile people of color and immigrants. This biased practice does not advance safety rather it wastes time and inflicts harm on BIPOC communities. We need a new vision of community safety and reinvestment to keep our families of color safe and together.*  
...
```{r table, child= 'report_embeds/report_traffic_type.Rmd'}
```
...
```{r bubble-chart, child= 'report_embeds/report_traffic_result_race.Rmd'}
```

```{r facet, child= 'report_embeds/report_traffic_result_citation_race.Rmd'}
```
...
```{r bubble-chart, child= 'report_embeds/report_traffic_result_citation_window_race.Rmd'}
```
