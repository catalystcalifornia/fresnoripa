...
ggplot(df, aes(x = stop_result_simple, y = rate))+
  ...
  geom_text(aes(label = paste0(round(rate,1), "%")), #add percentage labels within the bars
          ...
          color = "white")+ 
  scale_x_discrete(labels = ~ str_wrap(as.character(.x), 10))+
  labs(title=str_wrap("People Perceived as SWANA/SA are Given Citations at Higher Rates than Other Groups", 90),
...
df<-df%>%
  select(offense_code,offense_statute,statute_literal_25)%>%
  mutate(total=n())%>%
  group_by(offense_code,offense_statute,statute_literal_25)%>%
mutate(count=n(),
       rate=count/total*100)%>%
  slice(1)%>%
  arrange(-rate)%>%
