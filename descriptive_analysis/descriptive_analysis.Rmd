...
ggplot(df0, aes(x = reorder_within(statute_literal_25, -rate, Race), y = rate))+
  ...
  theme_bw()+
  theme(
    # axis.text.x = element_text(angle = 0, hjust =0.5,size=4), #sets x-axis labels to be horizontal
      # legend.position ="none", #removes legend
      plot.title = element_text(hjust=0.5),
      plot.subtitle = element_text(hjust=0.5))+
  theme(axis.text.x = element_text(vjust = 0.7, hjust=0.5, size=4), #aesthetics of the barchart
        strip.background = element_rect(fill = "#362178"),
