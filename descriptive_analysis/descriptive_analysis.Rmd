...
Out of all officer-initiated traffic stops made by Fresno PD, 77.2% (3,876) are moving violations, 14.8% (741) are equipment violations, and 8.1% (406) are non-moving violations. 
...
df<-traffic%>%
  left_join(codes, by=c("rfs_traffic_violation_code"="offense_code")) # get offense codes
...
colformat_double(j = c("Rate"), digits = 1)%>%
  set_caption('Top 3 Reasons for an Officer-Initiated Traffic Stop by Traffic Stop Type') %>%
  theme_vanilla() %>%
  width(j=3,2,unit="in")
...
df<-traffic%>%
  ...
  mutate(count=n(),
         rate=count/total*100)%>%
  slice(1)%>%
  select(traffic_violation_type, offense_statute, statute_literal_25, total, count,rate)%>%
  arrange(-rate)
