...
df0<-uof%>%
  ...
  mutate(Total=n())%>%
  group_by(reason, rfs_traffic_violation_code.x,statute_literal_25,offense_type_of_charge)%>%
  mutate(Count=n(),
         Rate=Count/Total*100)%>%
  slice(1)%>%
...
Fresno PD disproportionately inflicted use of force towards people they perceived as Latinx men in 2022. Latinx people make up 50% of the total population in Fresno City. Additionally, 17.6% of use of force incidents were towards people perceived as Asian males despite Asian people only making up ~13% of Fresno's total population. 
...
The table below shows the rate of searches that yield contraband within each perceived racial group (also known as hit rates). We can see across all perceived racial groups that only a small number of searches yield any contraband. 
...
names(df_hitrate)<-tools::toTitleCase(gsub("_"," ",names(df_hitrate))) # clean up column names for table
 ...
 colformat_double(j = c("Rate"), digits = 1)%>%
  set_caption('Fresno PD Searches Yielding Contraband by Perceived Race') %>%
  theme_vanilla()%>%
       width(width = 1.1)
