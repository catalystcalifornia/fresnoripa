...
In 2022, 94.6% of Fresno PD stops were officer initiated stops while only 5.4% of stops were calls for service.
...
The majority of people living in Fresno City are Latinx. However, there is also a diverse Southeast Asian communtiy that resides in Fresno. Unfortunately, due to the lack of available disaggregated data, it is difficult to discern the population of the Southeast Asian community in Fresno as their data is grouped into a larger 'Asian' category in the census.
...
In 2022, people perceived by Fresno PD as men (males) accounted for nearly 57% of all officer-initiated stops.
...
People perceived as Black by Fresno PD are stopped for traffic at disproportionately high rates relative to the total Black population in Fresno city. 
...
Note that the graph below filters for perceived racial groups that had 10 or more people stopped in traffic stops, and at least a 1% rate for officer-initiated traffic stops in 2022. 
...
ggplot(gender, aes(x = Gender, y = rate))+
  geom_bar(stat = "identity", fill = "#362178") + #to create barplot
  geom_text(aes(label = paste0(round(rate,1), "%")), #add percentage labels within the bars
          position = position_stack(vjust=1.1), #center percentage within the bar
          size = 2,
          color = "black")+
  scale_x_discrete(labels = ~ str_wrap(as.character(.x), 5))+
...
Within traffic stops that were made by Fresno PD, the most common cited reason is driving at an unsafe speed. However,  people perceived as Black and Latinx are stopped in traffic for a reason of 'No vehicle registration' at higher rates compared to other groups. This is noteworthy as vehicle registration, or lacktherof, does not advance public safety. Traffic stops made for these type of reasons may indicate that profiling is occurring.
...
ggplot(df0, aes(x = statute_literal_25, y = rate))+
  ...
  labs(title=str_wrap("People perceived as Black and Latinx are Stopped for Reasons Not Related to Traffic Safety",70),
...
The Fresno PD only stopped 9 people in 2022 for driving without a license.
...
Despite being stopped at higher rates than other perceived groups, people Fresno PD perceived as Latinx and Black are stopped for traffic stops resulting only in a warning verbal or written at higher rates than people perceived as White. 
...
ggplot(df, aes(x = stop_result_simple, y = rate))+
  ...
  geom_text(aes(label = paste0(round(rate,1), "%")), #add percentage labels within the bars
          ...
          color = "white")+ 
  scale_x_discrete(labels = ~ str_wrap(as.character(.x), 10))+
  labs(title=str_wrap("People Percevied as White are given Citations at Higher Rates than Other Groups", 90),
         subtitle = "Rate of traffic stops by stop result and perceived race",
       x = "", #labels to visual
       y = "") +
  theme_bw()+
  theme(axis.text.x = element_text(angle = 0, hjust =0.5), #sets x-axis labels to be horizontal
...
People perceived as Latinx are stopped for traffic stops and given a citation as a result at the highest rates compared to all other groups. However, people perceived as Black are disproportionately stopped in traffic stops and given a citation relative to how many Black people live in Fresno city. Out of all people stopped in traffic stops and given a citation, 11.4% were people Fresno PD perceived as Black, yet Black people make up 6.2% of Fresno city's total population. 
...
ggplot(df, aes(x = statute_literal_25, y = rate))+
  ...
  labs(title=str_wrap("Stops of People Perceived as Latinx and Black More Likely to Result in Citation for Driving Without License",90),
       subtitle = str_wrap("Top three traffic citation results by perceived race of person stopped",110), 
       x = "", #labels to visual
       y = "") +
  theme_bw()+
  theme(axis.text.x = element_text(angle = 0, hjust =0.5), #sets x-axis labels to be horizontal
...
People Fresno PD perceive as Latinx and Black are stopped in traffic stops and given a citation for driving without a license at higher rates than other groups. The percentage of people Fresno perceived as Black who were stopped and given a citation for driving without a license is disproportionately high relative to the percentage of Black people who live in Fresno city. 
...
df0<-uof%>%
  ...
  ungroup()%>%
  select(rfs_traffic_violation_code.x, statute_literal_25, Count, Rate)%>%
...
Fresno PD disproportionately inflicted use of force towards people they perceived as Latinx men in 2022. Latinx people make up 50% of the total population in Fresno City. Additionally, 18% of use of force incidents were towards people perceived as Asian despite Asian people only making up ~13% of Fresno's total population. 
...
Two officer-initiated stops, with a total of four people stopped, resulted solely in a field interview card being completed. All four FI cards were given to people during traffic stops. 
...
In 2022, Fresno PD made 68 searches during officer-initiated traffic stops and searched 59 people. Of the 59 people Fresno PD searched in traffic stops, 32 people were perceived as Latinx and 9 people were perceived as Black.  

Out of the 68 searches made in traffic stops, 50 of those searches did not yield any contraband. In other words, 73.5% searches made in traffic stops did not prevent any crime.
