...
Officer-Initiated Stops by the Fresno Police Department disproportionately target Latinx residents.  
...
ggplot(final_race, aes(x = reorder(Race, -Percent), y = Percent, fill = Race))+
  ...
  geom_text(aes(label = label), #add percentage labels within the bars
          ...
          size =3,
          color = "white")+ 
  labs(title = "Latinx Residents Make Up More than Half of Officer-Initiated Stops by Fresno PD",
...
Individuals aged 25-34 are most likely to experience Officer-Initiated Stops by Fresno PD.
...
ggplot(age_ois, aes(x = Age, y=Percent))+
  ...
  geom_text(aes( label = paste0(round(Percent, digits=1),"%" ), #round to tenths place within barchart label
    ...
    labs(title = "Residents Aged 25-34 Account for Nearly 35% of Officer-Initiated Stops by Fresno PD",
         subtitle= "Officer-Initiated Stops by Age Group",
      x = "", #labels
  y = "")+
...
age_race_gender_table <-age_race_gender_ois%>%
    filter(call_for_service ==0)%>% #Filtered for OIS
  group_by(age_group_re)%>%


final_sswana <- age_race_gender_table %>% #creates df for sswana ###WORK IN PROGRESS
  mutate(Total=n())%>%
  group_by(sswana_flag)%>%
  mutate(Count=n(), Percent = round(Count/Total*100, 1))%>% 
  slice(1)%>%
  arrange(-Percent)%>%
  ungroup()%>%
  select(age_group_re, sswana_label, Total, Count, Percent)
...
age_race_gender_ois_table<-age_race_gender_ois%>%
  ...
  group_by(age_group_re, nh_race,g_full,total_age, sswana_flag, sswana_label)%>% # Grouping by all 3 variables and keeping the total per age group recoded
