...
Out of all officer-initiated stops made by Fresno PD, 99.7% of stops were indicated as having no instance of any use of force (5,043 total stops) while 0.33% of stops were indicated in the data as having instances of use of force (17 stops).
...
Just two stops by Fresno PD in 2022 resulted solely in a field interview card being completed.
...
Fresno PD __only made four stops__ in 2022 that resulted in officers completing and filing a field interview card. All four of these stops were due to perceived traffic violations.
...
Of the five individuals that Fresno PD completed field interview cards for in 2022, four were identified by officers as Black or Latinx. Officers also identified three of these individuals as men and two as women.
...
ggplot(count_tv_reason, aes(x = traffic_violation_type, y = Percent,fill=traffic_violation_type)) +
  ...
  geom_text(aes(label = paste0(Count," people (",round(Percent,1),'%)')), #add percentage labels within the bars
          position = position_stack(vjust=0.5), #center percentage within the bar
          size =3,
          color = "white")+ 
  labs(
...
### Stop reasons within equipment/non-moving traffic stops
...
statutes<-dbGetQuery(con, "SELECT offense_code,statute_literal_25,offense_type_of_charge FROM cadoj_ripa_offense_codes_2023")

##Create data frame for table visualization with top 10 citations##
tv_citation_flex_qa <- tv_citation_count %>%
  left_join(statutes,
            by=c("rfs_traffic_violation_code"="offense_code"))
...
names(df)<-tools::toTitleCase(sub("_"," ",names(df))) # clean up column names for table

df<-df%>%
  rename("Statute"="Statute Literal_25")
