...
ggplot(df_race, aes(x = reorder(Race, rate), y = rate))+
  ...
  geom_text(aes(label = paste0(round(rate,1), "%")), #add percentage labels within the bars
          ...
          color = "white")+ 
  scale_x_discrete(labels = ~ str_wrap(as.character(.x), 5))+
  labs(title = "Half of the people Fresno PD stops for traffic stops are people perceived as Latinx", 
...
Note that the graphs below filter out perceived racial groups where less than 10 people wree stopped in traffic stops by Fresno PD in 2022. 
...
Despite being stopped at higher rates than over perceived groups, people Fresno PD perceives as Latinx and Black are stopped for traffic stops resulting only in a warning verbal or written at higher rates than people perceived as White. 
