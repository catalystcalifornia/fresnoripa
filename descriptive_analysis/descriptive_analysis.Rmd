...
##VISUALIZATION FOR RATE OF OIS BY AGE GROUP##
  ...
  ggplot(age_ois, aes(x = Age, y=Percent))+
      geom_bar( stat = "identity", fill="navy") + #create barchart 
  geom_text(aes( label = paste0(round(Percent, digits=1),"%" ), #round to tenths place within barchart label
    ...
    labs(title = "Officer-Initiated Stops by Age Group",
      x = "Age Group", #labels
  y = "Percent of Officer-Initiated Stops (%)")+
    scale_x_discrete(labels = wrap_format(10))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=10), #aesthetics of the barchart
...
##RECODING FOR GENDERSO THAT X-AXIS DISPLAYS GENDERS INSTEAD OF 1:5##
...
ggplot(gender_df, aes(x= reorder(value, Percent), y= Percent, fill = value))+
      geom_bar( stat = "identity", fill="navy") + #create barchart 
  geom_text(aes(label = paste0(round(Percent, digits=1),"%"), #round to tenths place within barchart label
                 y= Percent),
            vjust = 1.4, 
  ...
  y = "Percent of Officer-Initiated Stops (%)")+
    scale_x_discrete(labels = wrap_format(10))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1, size=7), #aesthetics of the barchart
        strip.background = element_rect(fill = "navy"),
        strip.text = element_text(color = "white", size=10, face="bold"),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
...
combined_df <- merge(gender_df, race_ois, by = "stop_id", all = TRUE)
combined_df <- merge(combined_df, age_ois, by = 'stop_id", all = TRUE)
