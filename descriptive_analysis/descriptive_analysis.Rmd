...
Within traffic stops that were made by Fresno PD, the most common cited reason is driving at an unsafe speed. However,  people perceived as Asian and Latinx are stopped in traffic for a reason of 'No vehicle registration' at higher rates compared to other groups. People perceived as Latinx were stopped because of an obstruction in vehicle window at higher rates than other groups. Traffic stops made for reasons such as vehicle registration, or lacktherof, do not advance public safety. Traffic stops made for these type of reasons may indicate that profiling is occurring.
...
df0<-traffic%>%
  ...
  group_by(nh_race)%>%
  mutate(total=n())%>%
  group_by(rfs_traffic_violation_code, statute_literal_25, offense_default_type_of_charge, nh_race)%>%
  mutate(count=n(),
         rate=count/total*100)%>%
  slice(1)%>%
   mutate(Race = case_when(
...
ggplot(df0, aes(x = reorder_within(statute_literal_25, -rate, Race), y = rate))+
  ...
  theme(axis.text.x = element_text(angle = 0, hjust =0.5), #sets x-axis labels to be horizontal
      ...
      plot.title = element_text(hjust=0.5),
      plot.subtitle = element_text(hjust=0.5))+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.7, hjust=0.5, size=3.5), #aesthetics of the barchart
        strip.background = element_rect(fill = "#362178"),
...
Individuals Fresno PD perceived as Latinx and Black are stopped for traffic stops resulting only in a warning verbal or written at higher rates than individuals Fresno PD perceived as White. 
...
df<-df%>%
  ...
  group_by(nh_race)%>%
  mutate(total=n())%>%
  group_by(offense_code, statute_literal_25, offense_default_type_of_charge, offense_type_of_charge,nh_race)%>%
  mutate(count=n(),
         rate=count/total*100)%>%
  slice(1)%>%
   mutate(Race = case_when(
...
ggplot(df, aes(x = reorder_within(statute_literal_25, -rate, Race), y = rate))+
  ...
  theme(axis.text.x = element_text(angle = 0, hjust =0.5), #sets x-axis labels to be horizontal
      ...
      plot.title = element_text(hjust=0.5),
      plot.subtitle = element_text(hjust=0.5))+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5, size=4), #aesthetics of the barchart
        strip.background = element_rect(fill = "#362178"),
        strip.text = element_text(color = "white", size=8, face="bold"),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
