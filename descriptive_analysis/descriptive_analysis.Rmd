...
##CREATE DF TO CREATE VISUAL##
counts_df <- bind_cols(count_ois, count_cfs)
ft <- flextable(counts_df)
...
* In 2022, there were 5,317 police stops made by Fresno PD. Of those stops, 5028 were officer-initiated stops and 289 were calls for service. 
...
ggplot(rate_ois, aes(x = Type, y = percent, fill = Type))+
  ...
  labs(title = "Rate of Stop Types (%)", #labels to visual
       x = "Type of Stop",
       y = "Percentage (%)") +
  scale_fill_manual(values = c("Officer-Initiated Stop"="navy",
      "Calls for Service" = "navy" ))+
  theme_bw()+
...
```
* In the year 2022, 94.6% of Fresno Police Department stops were officer initiated stops while only 5.4% of stops were calls for service.
...
race_ois<- left_join(stops, race, by ="stop_id")%>%
  filter(call_for_service ==0)%>%
  select(nh_race)%>% 
  mutate(Total=n())%>%
  group_by(nh_race)%>%
...
ggplot(race_ois, aes(x = reorder(Race, -Percent), y = Percent, fill = Race))+
  ...
  labs(title = "Officer-Initiated Stops by Race",
       x = "Race",
       y = "Percentage (%)") +
  scale_fill_manual(values = c("navy", "navy", "navy", "navy", "navy", "navy", "navy"))+
  theme_bw()+
  theme(plot.title = element_text(hjust =0.5),
 #sets y-axis labels to be horizontal
      legend.position ="none", #removes legend
      axis.text.y = element_text(angle =0, hjust=1), #Makes y-axis text readable
      axis.text.x = element_text(angle =0, hjust=0.5))+ #Makes x-axis text readable
  coord_flip() ##flips coordinates for readability

##VISUALIZATION FOR COUNT OF OIS BY RACE ##

ggplot(race_ois, aes(x = reorder(Race, Count), y = Count, fill = Race))+
  geom_bar(stat = "identity") + #to create barplot
  geom_text(aes(label = Count), #add percentage labels within the bars
          position = position_stack(vjust=0.5), #center percentage within the bar
          size =5,
          color = "white")+ 
  labs(title = "Officer-Initiated Stops by Race",
...
age_ois<- left_join(age, stops, by ="stop_id")%>%
  filter(call_for_service ==0)%>%
  select(age_group_re)%>% 
  mutate(Total=n())%>%
...
gender_ois <-left_join(persons, stops, by = "stop_id")%>%
  mutate(
    has_perceived_gender = rowSums(select(., g_male, g_female, g_transgender_man, g_transgender_woman, g_gender_nonconforming, g_lgbt)%>%
                                     filter(has_perceived_gender)%>%
                                     summarise(
  g_male_count = sum(g_male),
  g_female_count = sum(g_female),
  g_transgender_woman_count =sum(g_transgender_woman),
  g_transgender_man = sum(g_transgender_man),
  g_gender_nonconforming = sum(g_gender_nonconforming),
 g_lgbt = sum(g_lgbt))
